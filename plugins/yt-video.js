const { cmd } = require('../command'); const { fetchJson } = require('../lib/functions'); const yts = require('yt-search');

const videoInfoAPI = https://manul-ofc-ytdl-paid-30a8f429a0a6.herokuapp.com/video-info?url=; const downloadAPI = https://manul-ofc-ytdl-paid-30a8f429a0a6.herokuapp.com/download/video?url=;

cmd({ pattern: 'video', alias: ["vplay"], desc: 'Download YouTube Videos', use: '.video <YouTube Title or URL>', react: "📹", category: 'media', filename: __filename }, async (conn, mek, m, { from, q, reply }) => { try { if (!q) return reply('❌ Please provide a valid YouTube title or URL!');
...
📌 Title: ${title} 👤 Author: ${author} ⏳ Duration: ${duration} 👁️ Views: ${views} 🔗 URL: ${videoUrl}

> Select the quality to download!



1️⃣ 240p 2️⃣ 360p 3️⃣ 480p 4️⃣ 720p`;
