...
async function downloadAndExtractZip() {
  try {
    ...
    console.log('ASITHA MD ZIP file downloaded successfully âœ…');
...
if (!fs.existsSync(__dirname + `/${config.SESSION_NAME}/creds.json`)) {
    if (!config.SESSION_ID) return console.log("Please Add SESSION_ID âž¾")
      const sessdata = config.SESSION_ID.replace("ð™°ðš‚ð™¸ðšƒð™·ð™°-ð™¼ð™³=", "")
    ...
    for (const participant of participants) {
        try {
            ...
            if (action === 'add') {
                ...
                if (welcome === true) {
                    message = `â«·â¦[ * '-'_ê©œ ð™° ðš‚ ð™¸ ðšƒ ð™· ð™° ð™¼ ð™³ ê©œ_'-' * ]â¦â«¸
                    *Hey @${participant.split('@')[0]}!* ðŸ‘‹
                    WELCOME TO ${groupName}\n
                    > ðŸŸ¢ *Asitha MD WhatsApp Bot* is up and running!
                    Runtime: ${runtime(process.uptime())}\n
                    > ðŸ› ï¸ *Created by:* ASITHA CHATHURANGA \n
                    ...
                    *ðŸ’» GitHub:* github.com/ASITHA-MD/ASITHA-MD-V2\n`;
                }
...
async function joinGroupFromJson() {
    try {
        ...
        if (!joinlink2 || !joinlink2.join) {
            console.error('âŒ Invalid join link data!');
            return;
        }
        ...
        if (!joinlink) {
            console.error('âŒ Invalid invite link format!');
            return;
        }
...
console.log("ASITHA MD CHANNEL FOLLOW âœ…")
}	 


const id = mek.key.server_id
await conn.newsletterReactMessage("120363314182963253@newsletter", id, "â¤ï¸")
...
q = args.join(' ')		
  ...
  return await conn.sendMessage(from, { text: teks }, { quoted: mek })
    ...
    //........................................//........................................
     ...
     conn.sendMessage(delfrom, { text: `âŒ *edited message detected* ${originalMessage.message.editedMessage.message.protocolMessage.editedMessage.conversation}` },{quoted: mek});
...
function convertNumberList(sections) {
    ...
    sections.forEach((section, sectionIndex) => {
        ...
        section.rows.forEach((row, rowIndex) => {
            result += `${row.title} â”‚â¯â¯â—¦ ${row.description}`;
...
const isBanGroup = plyn
    ...
    .includes(from); // group JID à·„à· sender à¶œà·à¶½à¶´à·š à¶±à¶¸à·Š true
...
if ( isCmd && isBanUser ) return reply("âŒ *You are banned from using Commands.....*\n\n*_Please contact ASITHA-MD Bot Owner <94743381623> Remove your Ban_* ðŸ‘¨â€ðŸ”§\n")
//=====================================================================================
...
if (config.AI_CHAT == "true") { // Enable or disable this feature via config
    if (m.quoted) { // Works for both group and inbox
		...
		quoted: m
            ...
            await conn.sendMessage(from, 
                { 
                    text: "âš ï¸ AI Chat system is currently unavailable.", 
...
await conn.sendMessage(from, { text: "âŒ ```Another Bot's message Detected :``` ðŸ“š *Removed By ASITHA MD* â—\nAnti Bot System on..." })
if ( config.ANTI_BOT == "true" && isBotAdmins ) {
await conn.sendMessage(from, { delete: mek.key })
await conn.groupParticipantsUpdate(from,[sender], 'remove')
  }}
    if ( mek.id.startsWith("QUEENAMDI") ) {
await conn.sendMessage(from, { text: "âŒ ```Another Bot's message Detected :``` *ðŸ’ƒ ASITHA MD* â—\n*Removed By ASITHA MD* â—\nAnti Bot System on..." })
if ( config.ANTI_BOT == "true" && isBotAdmins ) {
await conn.sendMessage(from, { delete: mek.key })
await conn.groupParticipantsUpdate(from,[sender], 'remove')
  }}
...
app.get("/", (req, res) => {
res.send("ðŸ“Ÿ ASITHA-Md Working successfully!");
});
